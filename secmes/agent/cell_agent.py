...
from secmes.da.dgd import generate_linear_desc_array, generate_random_doubly_stoch_mat, grad_f, iteration_step
...
class CellAgentRole(SyncAgentRole, ABC):
    def __init__(self, network_component) -> None:
        super().__init__()
    ...
    def setup(self):
        self.context.subscribe_message(self, self.handle_join_request, lambda c,_: isinstance(c, JoinRequest))
        self.context.subscribe_message(self, self.handle_balance_answer, lambda c,_: isinstance(c, BalanceSumAnswer))
        self.context.subscribe_message(self, self.handle_balance_request, lambda c,_: isinstance(c, BalanceSumRequest))
        self.context.subscribe_message(self, self.handle_calc_op, lambda c,_: isinstance(c, CalcOperationPointIteration))

        region_m : SecmesRegionManager = self.context.region_manager
        router : SecmesAgentRouter = self.context.router
        neighbors = router.lookup_neighbors(self.context.aid)
        neighbor_regions = set([region_m.get_agent_region(neighbor) for neighbor in neighbors if region_m.get_agent_region(neighbor) is not None])
    ...
    def handle_balance_request(self, _, meta):
        router : SecmesAgentRouter = self.context.router
        router.dispatch_message(BalanceSumAnswer(self.calc_balance()), self._get_sender_id(meta), self.context.aid)
        pass
    ...
    def get_or_request_balance(self, agent_id):
        ...
        router : SecmesAgentRouter = self.context.router
        router.dispatch_message(BalanceSumRequest(), agent_id, self.context.aid)
    ...
    def project(self, x, r):
        cons_x = self.define_local_constraints()(x/(r/10 + 1))
    ...
    def handle_calc_op(self, content: CalcOperationPointIteration, _):
        ...
        w = content.w
        x = content.x
        region_agents = self.context.region_manager.get_agents_region(self.context.region_manager.get_agent_region(self.context.aid))
        i = region_agents.index(self.context.aid)
        f = lambda x: self.max_energy() * x
        new_x = iteration_step(
                w, 
                x, 
                lambda x: self.max_energy(), 
                len(region_agents), 
                i,
                time)
    ...
    def calculate_operation_point(self, time):
        ...
        for j in range(100):  
            new_x = iteration_step(
                    ...
                    j)
            for region_agent in region_agents:
                router.dispatch_message(CalcOperationPointIteration(time, w, alpha, x), region_agent, self.context.aid)
            x[i] = self.project(new_x, r)
    ...
    def execute_operation_point(self, time):
        if isinstance(self._local_model, RegulatableMESModel):
            ...
            self._local_model.regulate(operation_point) 
    ...
    def control(self, time):
        ...
        for neighbor in neighbors:
            attraction = self.calc_agent_attraction(neighbor, calculated_region_balance)
            ...
            if (attraction <= calculated_region_balance).all():
                router.dispatch_message(JoinRequest(region, attraction), neighbor, aid)
...
def power_constraint(bus_data, r):
    if np.abs(VM_PU_REF - bus_data['power'][0]['vm_pu']) * (r/10 + 1) <= 0.1:
...
def heat_constraint(junc_data, r):
    if TEMP_K_REF * 0.9 <= junc_data['heat'][1]['t_k'] * (r/10 + 1) <= TEMP_K_REF * 1.1:
...
def gas_constraint(junc_data, r):
    if P_BAR_REF * 0.9 <= junc_data['gas'][0]['p_bar'] * (r/10 + 1) <= P_BAR_REF * 1.1:
...
class PowerCellAgentRole(CellAgentRole):
    def max_energy(self):
        return to_multi_energy(self._local_model.active_power_capability())

    def define_local_constraints(self):
        return lambda r: power_constraint(pn.get_bus_junc_res_data(self._local_model), r)
...
class DummyHeatCellAgentRole(CellAgentRole):
    def max_energy(self):
...
class HeatCellAgentRole(CellAgentRole):
    def max_energy(self):
...
class PowerGasCellAgentRole(CellAgentRole):
    def max_energy(self):
        return to_multi_energy(power=self._local_model.active_power_capability(), gas=self._local_model.mdot_kg_per_s_capability())
    ...
    def calc_balance(self):
        ...
        return to_multi_energy(power=data['power'][0]['p_mw'], gas=self._local_model.mdot_kg_per_s())

class PowerGasHeatCellAgentRole(CellAgentRole):
    def max_energy(self):
        return to_multi_energy(power=self._local_model.active_power_capability(), 
                               gas=self._local_model.mdot_kg_per_s_capability(),
                               heat=self._local_model.q_capability())

    def define_local_constraints(self):
        data = pn.get_bus_junc_res_data(self._local_model)
        return lambda r: gas_constraint(data, r) and power_constraint(data, r) and heat_constraint(data, r)
    ...
    def calc_balance(self):
        ...
        return to_multi_energy(power=data['power'][0]['p_mw'], heat=from_data_set_heat['t_k'] - to_data_set_heat['t_k'], gas=self._local_model.mdot_kg_per_s())

class PowerHeatCellAgentRole(CellAgentRole):
    def max_energy(self):
        return to_multi_energy(power=self._local_model.active_power_capability(), 
                               heat=self._local_model.q_capability())
    ...
    def calc_balance(self):
        ...
        from_data_set_heat = data_heat[0]
        to_data_set_heat = data_heat[1]
        return to_multi_energy(power=data['power'][0]['p_mw'], heat=from_data_set_heat['t_k'] - to_data_set_heat['t_k'])

class GasCellAgentRole(CellAgentRole):
    def max_energy(self):
