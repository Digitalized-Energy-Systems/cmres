...
# ── Single-grid mode ──────────────────────────────────────────────────────────
# GRID_NAME is resolved inside run_simulation.py (single source of truth in
# ALL_GRIDS). The launcher just submits a 1..N_SHARDS array and exports the
# name to the worker via CMRES_GRID_NAME. GRID_IDX is the numeric alternative.
...
if [[ -n "${GRID_NAME}" ]]; then
    RUN_ARRAY="1-${N_SHARDS}"
    MERGE_ARRAY="1-1"
    TOTAL_TASKS=${N_SHARDS}
    export CMRES_GRID_NAME="${GRID_NAME}"
elif [[ -n "${GRID_IDX}" ]]; then
...
if [[ -n "${GRID_NAME}" ]]; then
    echo "  Single grid   : ${GRID_NAME} (resolved by run_simulation.py)"
elif [[ -n "${GRID_IDX}" ]]; then
    echo "  Single grid   : #${GRID_IDX}"
fi
...
# ── MERGE phase ───────────────────────────────────────────────────────────────
if [[ -n "${GRID_NAME}" ]]; then
    MERGE_HELP=( "--name ${GRID_NAME}" )
elif [[ -n "${GRID_IDX}" ]]; then
    MERGE_HELP=( "${GRID_IDX}" )
else
    MERGE_HELP=( $(seq 1 ${N_GRIDS}) )
fi
...
if [[ "${SUBMIT_MERGE}" != "1" ]]; then
    ...
    for sel in "${MERGE_HELP[@]}"; do
        echo "  python ./experiments/re/run_simulation.py ${sel} --merge"
    done
    exit 0
fi
...
if [[ -z "${MERGE_JOBID}" ]]; then
    ...
    for sel in "${MERGE_HELP[@]}"; do
        echo "         python ./experiments/re/run_simulation.py ${sel} --merge" >&2
    done
    exit 1
fi
